...
lv.setOnItemClickListener(new AdapterView.OnItemClickListener() {
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        ...
        btTambah.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                startActivity(new Intent(AdminActivity.this,InputAdminActivity.class));
            }
        });
...
registerForContextMenu(lv);
    ...
    private void tampilDetail(long id, int position) {
        ...
        Intent tampil = new Intent(AdminActivity.this,DetailActivity.class);
    ...
    private void hapus(final int position) {
        ...
        hapus.setTitle("Hapus").setMessage("Yakin ingin menghapus data ini ??").setCancelable(true).setPositiveButton("Hapus", new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int which) {
                final String hapus = link+"pln2/hapus.php?idpengukuran="+objectnyas.get(position).getIdnya();
                    swipeRefreshLayout.setRefreshing(true);
                JsonObjectRequest jhapus = new JsonObjectRequest(hapus, null, new Response.Listener<JSONObject>() {
                    @Override
                    public void onResponse(JSONObject response) {
                        try {
                            int sukses = response.getInt("sukses");
                ...
                AksesJson.getIntance().addToReqQueue(jhapus);
    ...
    private void tampilEdit(long id, int position) {
        Intent tampil = new Intent(AdminActivity.this,InputOperatorActivity.class);
        tampil.putExtra("kode_trafo",objectnyas.get(position).getKode());
        tampil.putExtra("idpengukuran",objectnyas.get(position).getIdnya());
        tampil.putExtra("feeder", objectnyas.get(position).getFeeder());
        tampil.putExtra("lokasi", objectnyas.get(position).getLokasi());
